...
void equate(matrix* m1, matrix* m2)
{
   ...
   matrix transpose(matrix m1) //Transpose Function
   {
       int i,j;
           matrix result;

           result = create_empty(m1.row_dim, m1.col_dim); //Creates matrix of row_dim rows and col_dim columns

           for (i=0; i<m1.row_dim; i++)
               for (j=0; j<m1.col_dim; j++)
               result.element[i][j] = m1.element[j][i];  //transpose: a[i][j] => a[j][i]
